...
组件化的开发的原理是基于一个基本的「模型」，为其添加不同的组件使其具备不同的功能。
组件化的开发是弱关联关系的，比如下面这样给 unity 中的摄像机加上刚体组件：
...
继承关系是无法做到这点的，因为继承的原则就是 **将一些列相同的方法抽象出来**，通过继承得到的物体是从一个模子里刻出来全部一样的东西。
而组件化的开发是为了解决差异化的问题，通过后期添加不同的组件使其成为不同的物体。

继承关系必须在子类实现差异化的方法，而组件化的开发则是把差异化也变成可以复用的东西。
下面通过 unity 的例子来证明这一点。
